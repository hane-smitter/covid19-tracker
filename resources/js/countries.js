...
var createCountryList = () => {
    const num_countries = country_list.length;
    let i = 0, ul_list_id;

    country_list.forEach((country, index) => {
        if((index % Math.ceil(num_countries / num_of_lists)) == 0) {
            ul_list_id = `list-${i}`;
            country_list_element.innerHTML += `<ul id="list-${i}"></ul>`;
            ++i;
        }
...
//do show countries when change is clicked
change_country_btn.onclick = () => {
    input.value = "";
    resetCountryList();
...
//close btn
close_list_btn.onclick = () => {
    search_country_element.classList.add('hide');
}

country_list_element.onclick = (e) => {
